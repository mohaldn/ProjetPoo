...
public class DominosGame extends JFrame { // Cette partie se lance dans une fenêtre
    ...
    JPanel panelPlateauView, panelDroit, panelGauche, learderboard; // Des panels utiles
    ...
    private JPanel createPanelDroit() { // Crée le panel de droite
        ...
        // Pas besoin de set button visible, c'est déjà fait par défaut
    ...
    private JPanel createPanelGauche() { // Crée le panel de gauche
        ...
        msgStatutView.setPreferredSize(new Dimension(120, 60));
        learderboard = createLeaderBoard();

        // AJout des élements dans le panel
        panelGauche.add(buttonStop);
        panelGauche.add(buttonRetourMenu);
        panelGauche.add(msgJoueurView);
        panelGauche.add(learderboard);
        return panelGauche;
    }

    private JPanel createLeaderBoard(){
        // Initialisation du leaderboard
        Joueur[] listLb = new Joueur[Liste_Joueurs.length];
        for(int i = 0 ; i < Liste_Joueurs.length ; i++){
            listLb[i] = Liste_Joueurs[i];
        }


        // Tri du tableau en fonction du score actuel
        for(int i = 0 ; i < listLb.length ; i++ ){
            int min = listLb[i].getScore();
            Joueur jmin = listLb[i];
            int index = i;
            for(int j = 0 ; j < listLb.length ; j++ ){
                if(listLb[j].getScore() < min ){
                    min = listLb[j].getScore();
                    Joueur temp = jmin;
                    jmin = listLb[j];
                    listLb[index] = jmin;
                    listLb[j] = temp;
        ...
        // Initalisation du panel
        learderboard = new JPanel(new GridLayout(listLb.length, 3));

        // Initialisation et ajout des éléments
        for(int i = 0 ; i < listLb.length ; i ++){
            // Initialisation des éléments
            JLabel j1 = new JLabel(""+(i+1));
            j1.setHorizontalAlignment(JLabel.CENTER); // Le texte est centré par rapport à la case
            JLabel j2 = new JLabel(""+listLb[i].getNom());
            j2.setHorizontalAlignment(JLabel.LEFT); // Le texte est centré par rapport à la case
            JLabel j3 = new JLabel("  :  "+listLb[i].getScore());
            j3.setHorizontalAlignment(JLabel.LEFT); // Le texte est centré par rapport à la case
    ...
    private boolean finDeTour() { // Gestion de la fin d'un tour
        ...
        panelGauche.remove(learderboard);
        panelGauche.add(createLeaderBoard());
